...
public class ScoringPointAction extends ModelDrivenBaseAction<TestAnswer> implements SessionAware, RequestAware {
	...
	public String submitAnswer() throws AppException, IOException, InterruptedException {
		...
		System.out.println("进入submitAnswer此方法");
		// 提交答案 进入评测
		...
		Runnable mthread = new Runnable() {
			...
			public void run() {
				...
				try {
					compileInfo = competeMainClassUtils.compileProgram(file2.getPath() + "\\", "Main");
				...
				try {
					testAnswer = testAnswerService.getById(answer.getId());
				} catch (AppException e2) {
					// TODO Auto-generated catch block
					e2.printStackTrace();
				}
					...
					try {
						...
						if (resultInfo.getExitVal() == 0) {
							...
							if (resultInfo.getExecTime() <= question.getRuntime()
									&& resultInfo.getAnswerOutput().equals(r.getO())) {
								// 答案正确
								System.out.println(r);
								scoringPoint.setScore(r.getS());
								scoringPoint.setResult("正确");
